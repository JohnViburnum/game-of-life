...
const mouseXcoordinate = document.querySelector('#mouseXcoordinate');
const mouseYcoordinate = document.querySelector('#mouseYcoordinate');
...
function changeCrdnts() { //-------- функция выявления изменения координат
  ...
  mouseXcoordinate.textContent = `X: ${x}`;
  mouseYcoordinate.textContent = `Y: ${y}`;
...
function xyStrCrdnts(a, b) { //------ функция преобразования длинных координат в короткие с аргументами порядка первых и вторых координат
  ...
  for (let i = 0; i <= crdntsA.max(); i++) { //--- цикл перебора для будущих неповторяющихся первых координат (по максимальной первой координате)
