//Кнопка сбросить колонию должна уметь деактивироваться
...
function startDraw() { //--------------- функция стартовой отрисовки канвас
...
function draw() { //----------------- функция отрисовки клеток
...
function clearGame() { //------------------------------ функция очистки мира
  ...
  startDraw();  //------------------------------------- запуск функции стартовой отрисовки канвас
  draw(); //------------------------------------------- запуск функции отрисовки клеток
  ...
  //shadowMode = false; //------------------------------- деактивировация режима тени
...
inptsColor.forEach(e => { //------------- для каждого поля ввода цветового оформления
  e.addEventListener('change', draw); //- при изменении: отрисовка клеток
});
...
Array.prototype.min = function () { //- установка метода возврата минимального значения для массива
  return Math.min.apply(null, this);
};
Array.prototype.max = function () { //- установка метода возврата максимального значения для массива
...
function strToArr() { //--------------- функция преобразования строки координат в массив координат
  ...
  let arr = strCrdnts.split(' '); //--- расщепление строки координат по ' ' (пробелам)
  let xArr = []; //-------------------- массив для X-координат
  let yArr = []; //-------------------- массив для Y-координат

  for (let item of arr) { //---- цикл перебора координат
    item = item.split(':'); //-------- расщепление координат по ':' (двоеточиям)
    xArr.push(item[0]); //------------ внесение X-координат в массив
    yArr.push(item[1].split(',')); //- расщепление Y-координат по ',' запятым ивнесение в массив
  }
  for (let yC of yArr) { //---------------- цикл перебора Y-координат
    for (let j = 0; j < yC.length; j++) //- цикл перебора подмассивов Y-координат
      if (yC[j].includes('-')) { //-------- если подмассив содержит '-' (тире), то
        yC[j] = yC[j].split('-'); //-------- расщепление подмассива на подподмассив
        for (let k = Number(yC[j][0]); k <= Number(yC[j][1]); k++) { //- цикл перебора числовых значений подподмасиива
          yC[j].push(k); //-------------------------------------------- внесение диапазона в подподмассив
        }
        yC[j].splice(0, 2); //---- удаление первых 2х значений (которые являлись обозначением диапазона)
      } else { //---------------- иначе (если подмассив не содержит' -' (тире),то)
        yC[j] = Number(yC[j]) //--- превращение подмассива в число
      }
  }
  let newYArr = []; //------------------------------------- массив для измененных Y-координат
  for (let i = 0; i < xArr.length; i++) { //--------------- цикл перебора X-координат
    for (let j = 0; j < yArr[i].length; j++) { //---------- цикл перебора подмассивоа Y-координат
      newYArr.push([Number(xArr[i]), yArr[i][j]]); //------ внесение X и Y-координат в массив
      for (let k = 0; k < yArr[i][j].length; k++) { //----- цикл перебора подподмассивов
        newYArr.push([Number(xArr[i]), yArr[i][j][k]]); //- внесение X и Y-координат в массив
      }
    }
  }
  for (const item of newYArr) { //- цикл перебора массива для измененных Y-координат
    if (!isNaN(item[1])) { //------ если Y-координата число, то
      arrCrdnts.push(item); //----- внесение координат в массив координат
    }
  }
  shadowMode = true; //- режим тени активирован
...
function drawPC() { //----------------- функция отрисовки канвас предпросмотра
  if (shadowMode) { //----------------- если режим тени активирован
    ...
    cWPC = сrdnsX.max() + 1; //- присваивание маскимальной координаты из X-координат + 1 (так как координаты начинаются с 0)
    cHPC = сrdnsY.max() + 1; //- присваивание маскимальной координаты из Y-координат + 1 (так как координаты начинаются с 0)
...
const arrCoursor = []; //- массив для координат курсора мыши
...
const targetProxy = new Proxy(targetObj, {
  set: function (target, key, value) {
    arrCoursor.push(value); //------------ добавление новых значений в массив при их изменении
    target[key] = value;
    return true;
  }
});
...
function changeCrdnts() { //-------- функция выявления изменения координат
  if (shadowMode) { //-------------- если режим тени активирован, то
    targetProxy.arrCoursor = x; //-- присвоение X-координаты курсора мыши объекту
    targetProxy.arrCoursor = y; //-- присвоение Y-координаты курсора мыши объекту
    if (arrCoursor.length > 4) { //- если массив координат курсора мыши больше 4, то
      arrCoursor.splice(0, 2); //--- удаление первых 2-х координат
    }
    //отлов момента изменения координаты
    if (arrCoursor[0] !== arrCoursor[2] || //- если X-кордината курсора мыши не равна предыдущему значению ИЛИ
      arrCoursor[1] !== arrCoursor[3]) { //--- Y-кордината курсора мыши не равна предыдущему значению, то
...
function drawShadow() { //----------- функция отрисовки тени колонии
  if (cWPC <= cW && cHPC <= cH) { //- если ширина и высота колонии меньше или равна ширине и высоте карты, то
    draw(); //----------------------- запуск функции отрисовки клеток (для очистки пройденного указателем мыши пути)
...
function horizReflection() { //-------- функция отражения координат колонии по горизонтали (слева направо)
  if (shadowMode) { //----------------- если режим тени активирован, то
    ...
    for (const item of arrCrdnts) { //- цикл изменения координат
      ...
      newArr.push([0 - item[0] + crdntX.max(), item[1]]); //- изменение координат и внесение в новый массив
    }
...
function vertReflection() { //--------- функция отражения координат колонии по вертикали (сверху вниз)
  if (shadowMode) { //----------------- если режим тени активирован, то
    ...
    for (const item of arrCrdnts) { //- цикл изменения координат
      ...
      newArr.push([item[0], 0 - item[1] + crdntY.max()]); //- изменение координат и внесение в новый массив
    }
...
let customCrdntsValidity = false; //------------------------------------- переменная проверки введенного текста в поле ввода пользовательских координат
...
btnSaveStr.addEventListener('click', () => { //- при нажатии на кнопку: сохранение пользовательской колонии
  if (inpCustomCrdnts.value === '') { //-------- если поле ввода пользовательских координат пусто, то
    ...
    customCrdntsValidityFunc(); //---------- запуск функции проверки правильности введенных координат
    if (customCrdntsValidity) { //---------- если переменная переменная проверки введенного текста в поле ввода равна true
      ...
      let code = //------------------------- текст информационного окна
        `<p><strong>Введенные координаты некорректны!</strong></p>
        <p>Вводите координаты сгенерированные в данном приложении.<br />
        Они должы иметь вид: <span class="code">"x:y,y,y x:y-y x:y,y-y"</span>,
        где <span class="code">x</span> – X-координата,
        <span class="code">y</span> – Y-координата, координаты разделены пробелами,
        X- от Y-координаты разделены двоеточием,
        Y-координаты записаны через запятую для одиночных Y-координат
        или через тире для диапазонов Y-координат, координаты не могут быть меньше 0 и больше 499</p>`
      dWkey = 'information'; //- обновление ключа для объекта типов информационного окна
      showWarning(code); //----- запуск функции показа информационного окна
    }
  }
...
icoInpCustomCrdnts.addEventListener('click', () => { //- при нажатии: показ информационного окна
  let code = //----------------------------------------- текст информационного окна
    `<p>Если у вас имеются координаты колонии сгенерированные в данном приложении в формате <span class="code">"x:y,y,y x:y-y x:y,y-y"</span>,
    где <span class="code">x</span> – X-координата, <span class="code">y</span> – Y-координата, координаты разделены пробелами,
    X- от Y-координаты разделены двоеточием, Y-координаты записаны через запятую для одиночных Y-координат
    или через тире для диапазонов Y-координат, координаты не могут быть меньше 0 и больше 499<br />
    Можете ввести их поле ввода и нажать "Cохранить". Если оставите поле ввода пустым, сохранятся координаты
    колонии находящейся на карте.</p>`
  dWkey = 'information'; //- обновление ключа для объекта типов информационного окна
  showWarning(code); //----- запуск функции показа информационного окна
});
...
function arrToStr() { //- функция преобразования координат
  ...
  for (const item of colony) { //- цикл изменения координат (сброс колонии к началу координат)
    ...
    arrCrdnts.push([item[0] - colonyX.min(), item[1] - colonyY.min()]); //- изменение координат и внесение в массив координат
  }
  //преобразование координат в короткие координаты
  const crdntsX = []; //------------- массив для длинных X-координат
  const crdntsY = []; //------------- массив для длинных Y-координат
  for (const item of arrCrdnts) { //- цикл перебора длинных координат
    crdntsX.push(item[0]); //-------- внесение в массив X-координат
    crdntsY.push(item[1]); //-------- внесение в массив Y-координат
  }
  const xClstr = []; //------------------------------------------- массив для неповторяющихся X-координат
  const yClstr = []; //------------------------------------------- массив для Y-координат сопряженных с X-координатами
  for (let i = 0; i < crdntsX[crdntsX.length - 1] + 1; i++) {  //- цикл перебора для будующих неповторяющихся X-координат (по максимальной X-координате)
    for (let j = 0; j < crdntsX.length; j++) { //----------------- цикл перебора X-координат
      if (crdntsX[j] === i) { //---------------------------------- если X-координата равна счету, то
        xClstr.push(i); //---------------------------------------- внесение счета в массив
        break; //------------------------------------------------- завершение итерации
      }
    }
    yClstr.push([]); //- генерация пустых подмассивов по количеству неповторяющихся X-координат
  }
  let yClstr2 = []; //---------------------------- массив для полных Y-координат
  for (let i = 0; i < yClstr.length; i++) { //---- цикл перебора массива для Y-координат сопряженных с X-координатами
    for (let j = 0; j < crdntsX.length; j++) { //- цикл перебора X-координат
      if (crdntsX[j] === i) { //------------------ если X-координата равна счету, то
        yClstr[i].push(crdntsY[j]); //------------ внесение Y-координат в массив для Y-координат сопряженных с X-координатами
      }
    }
    //сокращение Y-координат
    let sliceNum = [0]; //-------------------------- массив для адресов вырезания координат
    for (let j = 0; j < yClstr[i].length; j++) { //- цикл перебора массива для Y-координат сопряженных с X-координатами
      if (yClstr[i][j + 1] - yClstr[i][j] > 1) { //- если Y-координата меньше последующей более чем на 1, то
        sliceNum.push(j + 1); //-------------------- внесение счета + 1 в массив
      }
    }
    let shortY = []; //---------------------------------------------- массив для Y-координат
    for (let j = 0; j < sliceNum.length; j++) { //------------------- цикл перебора массива для адресов вырезания координат
      shortY.push(yClstr[i].slice(sliceNum[j], sliceNum[j + 1])); //- внесение Y-координат группами в массив
    }
    for (let j = 0; j < shortY.length; j++) { //---------------------------- цикл перебора массива для Y-координат
      if (shortY[j].length > 1) { //---------------------------------------- если длина Y-координаты больше 1, то
        shortY[j] = `${shortY[j][0]}-${shortY[j][shortY[j].length - 1]}` //- создание диапазона
      }
    }
    yClstr[i] = shortY; //-------------- Y-координата принимает массив
    yClstr[i] = yClstr[i].join(','); //- соединение Y-координат запятыми
    if (yClstr[i] !== '') { //---------- если Y-координата не пуста, то
      yClstr2.push(yClstr[i]); //------- внесение Y-координаты в массив для полных Y-координат
    }
  }
  let shortStrCrdnst = []; //----------------------------- массив для коротких координат
  for (let i = 0; i < xClstr.length; i++) { //------------ цикл перебора массива для неповторяющихся X-координат
    shortStrCrdnst.push(`${xClstr[i]}:${yClstr2[i]}`); //- внесение неповторяющейся X-координаты и полной Y-координаты в массив
  }
  strCrdnts = shortStrCrdnst.join(' '); //- строка координат принимает соединенный массив для коротких координат
}
...
function customCrdntsValidityFunc() { //---------------- функция проверки правильности введенных координат
  ...
  if (!(inpCustomCrdnts.validity.patternMismatch)) { //- если введенное в поле ввода соответсвтует html-паттерну, то
    ...
    for (const item of customStr) { //--- цикл перебора массива
      if (item < 0 || item > 499) { //--- если в массиве есть отрицательные числа ИЛИ числа больше 499, то
        customCrdntsValidity = false; //- переменная проверки введенного текста в поле ввода пользовательских координат присваивает false
      } else { //------------------------ иначе (если в массиве нет отрицательныx чисел ИЛИ числел больше 499, то)
        customCrdntsValidity = true; //-- переменная проверки введенного текста в поле ввода пользовательских координат присваивает true
      }
    }
...
//режим широкого формата вкладки колоний
const btnLandscapeMode = document.querySelector('#btnLandscapeMode'); //- кнопка изменения форматов вкладки колоний

btnLandscapeMode.addEventListener('click', () => { //------------- при нажатии: смена форматов вкладки
  if (document.body.clientWidth > 630) { //----------------------- если ширина элемента body больше 630
    if (document.body.classList.contains('landscape')) { //------- если элемент body содержит класс 'landscape', то
      ...
      btnLandscapeMode.textContent = 'Широкий формат вкладки'; //- обновление текста кнопки изменения форматов вкладки
      ...
      btnLandscapeMode.textContent = 'Узкий вормат вкладки'; //- обновление текста кнопки изменения форматов вкладки
      btnsTabDropDownColony.forEach(e => { //------------------- для каждой кнопки открытия/скрытия субпанелей вкладки колонии
    ...
    if (document.body.classList.contains('landscape')) { //------- если элемент body содержит класс 'landscape', то 
      ...
      if (screen.orientation.type === 'portrait-primary') { //- если ориентация экрана книжная, то
        code = //---------------------------------------------- обновление текста информационного окна
          `<p>Ширина экрана не позволяет показать широкий формат вкладки,
          попробуйте изменить положение экрана на альбомный</p>`
      } else { //- иначе (если ориентация экрана альбомная, то)
        code = //- обновление текста информационного окна
          `<p>К сожалению, ширина экрана не позволяет показать широкий формат вкладки</p>`
      }
      showWarning(code); //- запуск функции показа информационного окна
    }
  }
...
btnDropDownToggleSettings.addEventListener('click', () => { //-- при нажатии: сброс широкого формата вкладки
  if (document.body.classList.contains('landscape')) { //------- если элемент body содержит класс 'landscape', то
    document.body.classList.remove('landscape'); //------------- удаление класса 'landscape' для элемента body
    btnLandscapeMode.textContent = 'Широкий формат вкладки'; //- обновление текста кнопки изменения форматов вкладки
    btnsTabDropDownColony.forEach(e => { //--------------------- для каждой кнопки открытия/скрытия субпанелей вкладки колонии
